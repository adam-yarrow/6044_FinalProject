function H_k1 = compute_H_wrapper(tk, debris_x0, GPS_x, Receiver_x, ft, c, dT, processNoise)
...
% dT           : scalar time step
% processNoise : [n x 1] process noise vector (depends on model)
%
% Output:
% H_k1         : [N x n] stacked measurement Jacobian mapped to initial state
    ...
    for k = 1:length(tk)
        ...
        debris_x = OrbitalDynamics(tk(k), debris_x, dT, processNoise);
